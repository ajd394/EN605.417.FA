...
void RunExperement(const std::string filename){
    ...
    std::cout << std::endl << "Executing run of the following command: " << filename << std::endl;
    ...
    kernel = clCreateKernel(program, (filename + "_kernel").c_str(), NULL);
    if (kernel == NULL)
    {
...
///
//	main() for assignment10 program
//
int main(int argc, char** argv)
{
    std::string arr[] = {"add", "sub", "mul", "div","pow"};
    std::vector<std::string> kernels(arr, arr + sizeof(arr)/sizeof(std::string));

    std::for_each(kernels.begin(), kernels.end(), RunExperement);
    return 0;
}
